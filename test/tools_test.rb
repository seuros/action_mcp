...
class ToolsTest < ActiveSupport::TestCase
  test "AnalyzeCsvTool should return fake analysis results" do
    ...
    assert JSON.parse(result.first.text).keys.sort == %w[count sum]
  end
  ...
  test "ChecksumCheckerTool should validate file checksums" do
    ...
    result = tool.call
    assert result.first.is_a?(ActionMCP::Content::Text)
    assert result.first.text == "[#{'test.txt'.hash}]"
  end
  ...
  test "ExecuteCommandTool should return a fake execution result" do
    ...
    result = tool.call
    assert result.first.is_a?(ActionMCP::Content::Text)
    assert_match(%r{Executed: ls -l /home}, result.first.text)
  end
  ...
  test "FormatCodeTool should format code by trimming extra spaces" do
    ...
    result = tool.call
    assert result.first.is_a?(ActionMCP::Content::Text)
    assert_equal "def test end", result.first.text
  end
  ...
  test "GitHubCreateIssueTool should return a fake GitHub issue URL" do
    ...
    result = tool.call
    assert result.first.is_a?(ActionMCP::Content::Text)
    assert_match %r{https://github.com/fake/repo/issues/\d+}, result.first.text
  end
end
